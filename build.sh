...
# Build the image
docker build -t docknix-build -f Dockerfile.build . --build-arg DOCKNIX_RELEASE_COOKIE=$DOCKNIX_RELEASE_COOKIE \
...
-it --name docknix-build -v $(pwd)/_build/prod/rel:/opt/app/_build/prod/rel docknix-build
